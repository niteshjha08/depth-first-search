...
void rwa2::Mouse::trace_path(){
    ...
    std::stack<std::array<int,2>> v_s_temp=v_s;
    // // for(int i=0;i<15;++i)
    // //     move_forward();
    int len=v_s_temp.size();
    for(int i=0;i<len;++i){
        temp.push({v_s_temp.top()[0],v_s_temp.top()[1]});
        v_s_temp.pop();
    }
    ...
    //     // API::moveForward();
        ...
        if(!move_path_step(temp.top()[0],temp.top()[1]))
            {std::cerr<<"v_x at break is:"<<v_x<<"v_y at break is:"<<v_y<<" v_direction at break"<<v_direction;
            modify_maze(m_x,m_y,m_direction);
            std::stack<std::array<int,2>> empty{};
            API::clearAllColor();
            v_s=empty;
            if(v_new_search(1,13))
                trace_path();
            break;}
        temp.pop();
    }
